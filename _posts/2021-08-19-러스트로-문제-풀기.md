...
여러 줄에 걸친 출력을 수행할 때에는 `println!()`의 사용을 최소화해야 합니다. `println!()`을 사용하면 출력이 버퍼링되지 않기 때문입니다. 러스트 표준 라이브러리는 [표준 출력][1]을 줄 단위로 버퍼링하기 때문에 새줄 문자를 포함하는 `println!()`이 사용될 때마다 버퍼의 내용을 모두 `write` 시스템 호출을 통해 내보냅니다. 너무 빈번한 시스템 호출은 긴 실행 시간으로 이어질 수 있습니다.
...
이런 경우에는 해당 변수들을 묶어 구조체를 정의하면 구조체의 메서드 안에서 해당 변수들을 전역변수처럼 쓸 수 있습니다. (물론 `self`는 써야하지만요)
