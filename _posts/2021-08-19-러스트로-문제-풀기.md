...
여러 줄에 걸친 출력을 수행할 때에는 `println!()`의 사용을 최소화해야 합니다. `println!()`을 사용하면 출력이 버퍼링되지 않기 때문입니다. 러스트 표준 라이브러리는 [표준 출력][1]을 줄 단위로 버퍼링하기 때문에 (새줄 문자를 출력하는) `println!()`이 사용될 때마다 버퍼의 내용을 모두 내보냅니다(flush). 너무 빈번한 내보내기는 긴 실행 시간으로 이어질 수 있습니다.
